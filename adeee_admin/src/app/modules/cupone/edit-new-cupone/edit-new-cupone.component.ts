    ...
    showCupon(){
      this._cuponService.showCupon(this.cupone_id).subscribe(( resp:any) => {
        ...
        if(this.type_segment == 1){ 
          this.cupone_selected.products.forEach(products_s => {
            this.products.forEach(product => {
              if(product._id == products_s._id){
          ...
          this.cupone_selected.categories.forEach(products_s => {
    ...
    update(){
      ...
      this._cuponService.updateCupone(data).subscribe((resp:any) => {
        ...
        if(resp.message == 403){
          ...
          this.toaster.open(NoticyAlertComponent,{text:`success-'${resp.message_text}'`});
          return;
        }
      })
    }
